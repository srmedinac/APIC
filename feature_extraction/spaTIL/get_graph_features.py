...
def get_graph_features(x, y):
    ...
    for i, cell in enumerate(vor.regions):
        if cell and (np.any(np.isinf(vor.vertices[cell, :])) or np.any(np.isin(vor.vertices[cell, :], Vextreme))):
            banned.append(i)
    ...
    if len(banned) > len(vor.regions) - 2:
        banned = [i for i, cell in enumerate(vor.regions) if np.any(np.isinf(vor.vertices[cell, :]))]
    ...
    for i in range(len(tri.simplices)):
        ...
        sidelen[c:c+3] = [np.sqrt(np.sum((t[0] - t[1])**2)),
    ...
    for R in range(10, 60, 10):
        ...
        NNRR = np.array([len(np.where(distmat[i] <= R)[0]) - 1 for i in range(len(distmat))])
        ...
        if np.sum(NNRR) == 0:
            ...
            NNRR_dis[rcount - 1] = 1 - (1 / (1 + (NNRR_sd[rcount - 1] / NNRR_av[rcount - 1])))
