...
def getClosestNeighborsByGroup(groups, group_id):
    ...
    for j in range(num_groups):
        if group_id == j:
            dist = np.square(np.linalg.norm(groups[group_id]['clusterCentroids'], axis=1, keepdims=True)
                              - np.linalg.norm(groups[group_id]['clusterCentroids'], axis=1, keepdims=True).T)
            np.fill_diagonal(dist, np.inf)
        else:
            # Check if centroids array is not empty
            if not groups[group_id]['clusterCentroids'].any() or not groups[j]['clusterCentroids'].any():
                continue  # Skip empty centroids array
            
            # Use broadcasting to calculate distances
            dist = np.linalg.norm(groups[group_id]['clusterCentroids'][:, np.newaxis, :] - groups[j]['clusterCentroids'][np.newaxis, :, :], axis=2)
...
# Example usage:
# neighbors_result = getClosestNeighborsByGroup(groups_data, group_id)
