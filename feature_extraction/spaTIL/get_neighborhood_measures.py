    ...
    for i in range(num_groups):
                ...
                if num_rows > 0:
                    for k in range(1, neighborhood_size + 1):
                        if k <= max_num_clusters and k <= num_rows:
                            ...
                            val = np.array([item[0] for item in groups[i]['absoluteClosest'][j]['idx']])[:k]
        ...
        for st in range(num_stats):
            for k in range(neighborhood_size):
                for kk in range(num_groups):
                    feature_names.append(f"{stats[st]}PercentageClusters_G{kk + 1}_Surrounding_G{i + 1}_Neighborhood{k + 1}")
